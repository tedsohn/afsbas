    ...
    function (Controller, JSONModel, MessageToast, Filter, FilterOperator) {
        ...
        return Controller.extend("zmrctree.controller.View1", {
            ...
            onExpandFirstLevel: function () {
                ...
                // Update button text based on the new state
                var oToggleButton = this.byId("firstLevelExpandButton");
                if (oToggleButton) {
                    oToggleButton.setText(bFirstLevelState ? "First Level" : "Collapse");
                }
            ...
            onSaveAll: function (oEvent) {
                MessageToast.show("Node Hierarchy is being saved");
            },
...
onToggleRemoveColumn2: function () {
 ...
 onToggleRemoveColumn: function () {
    ...
    var iColumnIndexToRemove = 4;
    var oColumnToRemove = aColumns[iColumnIndexToRemove];

               // Toggle the firstLevelState
               var oGeneralModel = this.getView().getModel("generalModel");
               var bColumRemoveState = !oGeneralModel.getProperty("/columnRemoveState");

               // Set the new state
               oGeneralModel.setProperty("/columnRemoveState", bColumRemoveState);

               // Update button text based on the new state
               var oToggleButton = this.byId("removeColumnButton");
               if (oToggleButton) {
                   oToggleButton.setText(bColumRemoveState ? "Remove Keys" : "Restore");
               }

               // Perform the corresponding action based on the state
               var oTreeTable = this.byId("TreeTable");
               if (bColumRemoveState) {
                oTreeTable.removeColumn(oColumnToRemove);
               } else {
                oTreeTable.addColumn(oColumnToRemove);
               }


    // if (iColumnIndexToRemove !== undefined && iColumnIndexToRemove >= 0 && iColumnIndexToRemove < aColumns.length) {
    //     var oColumnToRemove = aColumns[iColumnIndexToRemove];
    // }
},
